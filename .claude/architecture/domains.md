...
#### SSL Provisioning System Redesign (v2 — DNS-01 + Async + Managed Nginx)
- **File(s):** Complete rewrite across 14 new files and 8 modified files
- **Type:** architecture redesign
- **Description:** Full redesign of SSL provisioning system replacing the synchronous webroot-based approach with async DNS-01 challenge and managed nginx directory. **New services:** (1) `AcmeService` — creates `_acme-challenge.{hostname}` TXT record via existing DnsProviderAdapter API, waits for propagation, runs `certbot certonly --manual --preferred-challenges dns-01`. Never touches nginx for ACME validation. (2) `NginxConfigService` — manages `/etc/nginx/openclaw-managed/` directory. One-time setup adds include to nginx.conf. Never touches sites-enabled/, conf.d/, or any existing configs. Each hostname gets its own `.conf` file with `# Managed-by: OpenClaw` header. (3) `ProvisioningRunner` — @Async state machine modeled after DeploymentJob/ScriptRunner. Steps: PENDING_DNS→DNS_CREATED→DNS_PROPAGATED→ISSUING_CERT→CERT_ISSUED→DEPLOYING_CONFIG→VERIFYING→COMPLETED with FAILED_RETRYABLE/FAILED_PERMANENT. Always cleans up TXT record in finally block. (4) `ProvisioningOrchestrator` — coordination layer called by controllers. Creates jobs, fires async runner, handles retry/cancel. (5) `SslVerificationService` — verifies HTTPS reachability + TLS cert validity via curl/openssl from remote server. **Schema changes:** `provisioning_jobs` table (V15 migration), `ssl_certificates.server_id` made nullable + `provisioning_job_id` added, unique index on active cert per assignment. **SslService** gutted to read/query service only (renew, remove, check). **SslController** now returns 202 Accepted with job ID for polling. **DomainAssignmentService** auto-triggers async provisioning instead of blocking.
- **Key decisions:** DNS-01 chosen over HTTP-01 (eliminates all port 80/443 conflicts with existing configs). Managed directory chosen over sites-enabled (no collision with manual or other-tool configs). Async chosen over sync (30-60s certbot + DNS propagation should not block HTTP request thread). TXT record always cleaned up in finally block.
- **Dependencies:** DnsProviderAdapter (TXT record support), SshService, ServerService, SslConfig, SecretService, ManagedZoneService, ProviderAccountService
- **Date:** 2026-03-24
