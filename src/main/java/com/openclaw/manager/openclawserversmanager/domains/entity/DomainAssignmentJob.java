...
@Entity
@Table(name = "domain_assignment_jobs")
public class DomainAssignmentJob {
    ...
    private DomainJobStep currentStep = DomainJobStep.PENDING_DNS;

    @Enumerated(EnumType.STRING)
    @Column(nullable = false, length = 20)
    private DomainJobStatus status = DomainJobStatus.RUNNING;
    ...
    public DomainJobStep getCurrentStep() { return currentStep; }
    public void setCurrentStep(DomainJobStep currentStep) { this.currentStep = currentStep; }

    public DomainJobStatus getStatus() { return status; }
    public void setStatus(DomainJobStatus status) { this.status = status; }
